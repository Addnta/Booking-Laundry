...
<main>
    <div class="page-header">
        <div>
            <h1>Edit Jadwal</h1>
            <p>Perbarui jadwal layanan dan kapasitas.</p>
        </div>
            ...
            <div class="mb-3">
                ...
                <select name="service_id" class="form-control" required>
                    ...
                    <?php foreach ($services as $sv) : ?>
                        <option value="<?= $sv['id'] ?>" <?= old('service_id', $schedule['service_id']) == $sv['id'] ? 'selected' : '' ?>><?= esc($sv['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label">Tanggal</label>
                <input type="date" name="date" class="form-control" min="<?= date('Y-m-d') ?>" value="<?= old('date', $schedule['date']) ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Slot Waktu (HH:MM)</label>
                <input type="time" name="time_slot" class="form-control" value="<?= old('time_slot', $schedule['time_slot']) ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Kapasitas</label>
                <input type="number" name="capacity" class="form-control" value="<?= old('capacity', $schedule['capacity']) ?>" required>
            </div>

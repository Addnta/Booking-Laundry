...
<head>
    ...
    <style>
        ...
        .service-preview img { max-height: 120px; }
...
<main>
    <div class="page-header">
        <div>
            <h1>Edit Layanan</h1>
            <p>Update informasi layanan yang sudah tersedia.</p>
        </div>
        <a href="<?= base_url('/admin/services') ?>" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i> Kembali</a>
    </div>
    ...
    <div class="card-panel">
        <form action="<?= base_url('/admin/services/update/' . $service['id']) ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" value="<?= esc(old('name', $service['name'])) ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Deskripsi</label>
                <textarea name="description" class="form-control"><?= esc(old('description', $service['description'])) ?></textarea>
            </div>
            <div class="mb-3">
                <label class="form-label">Harga</label>
                <input type="number" step="0.01" name="price" class="form-control" value="<?= esc(old('price', $service['price'])) ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Durasi (menit)</label>
                <input type="number" name="duration" class="form-control" value="<?= esc(old('duration', $service['duration'])) ?>" required>
            </div>
            <div class="mb-3 service-preview">
                <label class="form-label">Foto (opsional)</label>
                <?php if (!empty($service['photo']) && file_exists('uploads/services/' . $service['photo'])) : ?>
                    <div class="mb-2"><img src="<?= base_url('uploads/services/' . $service['photo']) ?>" alt="" class="img-fluid"></div>
                <?php endif; ?>
